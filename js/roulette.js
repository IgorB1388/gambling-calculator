...
const BetDefinitions = {
    red: { condition: (num, type) => num !== 0 && (!['american'].includes(type) || num !== 37) && [1,3,5,7,9,12,14,16,18,19,21,23,25,27,30,32,34,36].includes(num) },
    black: { condition: (num, type) => num !== 0 && (!['american'].includes(type) || num !== 37) && [2,4,6,8,10,11,13,15,17,20,22,24,26,28,29,31,33,35].includes(num) },
    even: { condition: (num, type) => num !== 0 && (!['american'].includes(type) || num !== 37) && num % 2 === 0 },
    odd: { condition: (num, type) => num !== 0 && (!['american'].includes(type) || num !== 37) && num % 2 === 1 },
    low: { condition: (num, type) => num !== 0 && (!['american'].includes(type) || num !== 37) && num <= 18 },
    high: { condition: (num, type) => num !== 0 && (!['american'].includes(type) || num !== 37) && num >= 19 },
    zero: { condition: (num, type) => num === 0 },
    doublezero: { condition: (num, type) => type === 'american' && num === 37 },
    dozen1: { condition: (num, type) => num >= 1 && num <= 12 },
    dozen2: { condition: (num, type) => num >= 13 && num <= 24 },
    dozen3: { condition: (num, type) => num >= 25 && num <= 36 },
    column1: { condition: (num, type) => num % 3 === 1 && num <= 34 },
    column2: { condition: (num, type) => num % 3 === 2 && num <= 35 },
    column3: { condition: (num, type) => num % 3 === 0 && num <= 36 && num !== 0 }
};
...
function updateStrategyOptions() {
    ...
    baseKeys.forEach(key => {
        ...
        option.value = key;
        option.textContent = getTranslation(`strategy${key.charAt(0).toUpperCase() + key.slice(1)}`);
...
function simulateSession(config, startingBankroll, betAmount, strategy, triggerType, triggerCount, stopBankrupt, stopTarget, targetAmount, stopSpins, maxSpins) {
    let bankroll = startingBankroll, spins = 0, triggerCounter = 0, betPlaced = false;
    while (true) {
        ...
        if (triggerCounter >= triggerCount && !betPlaced) {
            ...
            if (bankroll >= betAmount) {
                bankroll -= betAmount;
                if (isWin) bankroll += betAmount + (betAmount * (config.payouts[strategy] || 1));
            }
        }
...
function runSimulation(config, startingBankroll, betAmount, strategy, triggerType, triggerCount, stopBankrupt, stopTarget, targetAmount, stopSpins, maxSpins, sessionCount) {
    ...
    for (let i = 0; i < sessionCount; i++) {
        sessions.push(simulateSession(config, startingBankroll, betAmount, strategy, triggerType, triggerCount, stopBankrupt, stopTarget, targetAmount, stopSpins, maxSpins));
    }
    ...
    return { winRate: successfulSessions/sessionCount, avgProfit, riskRate: bankruptSessions/sessionCount, successfulSessions, bankruptSessions, totalSessions: sessionCount, avgSpins, maxProfit, maxLoss: Math.abs(minProfit), profitDistribution: sessions.map(s => s.profit).sort((a,b)=>a-b) };
}
...
function drawSimpleChart(distribution) {
    ...
    if (distribution.length===0){ chartContainer.innerHTML='<span class="text-muted" data-i18n="noData"></span>'; return; }
